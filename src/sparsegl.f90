...
SUBROUTINE sparse_four (bn,bs,ix,iy,gam,nobs,nvars,x,y,pf,pfl1,dfmax,pmax,nlam,&
      ...
      alsparse1,alsparse2,lb,ub,cn,drgix,drgiy,cn_s,cn_e)
  ...
  INTEGER:: nbeta(nlam)
  DOUBLE PRECISION :: alsparse1(nlam)
  DOUBLE PRECISION :: alsparse2(nlam)
  DOUBLE PRECISION :: flmin, eps, max_gam, d, maxDif, al, alf, snorm
  ...
  DOUBLE PRECISION:: tlam, lama1, lama2,lama3, al0
  INTEGER:: violation
  ...
  DO WHILE (l < nlam) !! This is the start of the loop over all lambda values...
     ...
     CALL strong_rule (is_in_S_set, ga, pf, tlam) !uses s_set instead of e_set...
     ! --------- outer loop ---------------------------- !
     DO
        CALL rchkusr()
        oldbeta(0) = b(0)
        ...
        DO
           ...
           DO g = 1, bn
              ...
              CALL update_step(bs(g), startix, endix, b, lama1,lama2,lama3, t_for_s(g),&
                   pf(g), pfl1(startix:endix), x,&
                   isDifZero, nobs, r, gam(g), maxDif, nvars, lb(g), ub(g), startsubgp=startsubgp, endsubgp=endsubgp, drgix=drgix, &
                   drgiy=drgiy, drgdim=cn)
        ...
        CALL strong_kkt_check(is_in_E_set, violation, bn, ix, iy, pf, pfl1,&
             bs, lama1,lama2,lama3, ga, is_in_S_set, x, r, nobs, nvars, vl,drgix,drgiy,cn,cn_s,cn_e,b) ! Step 3
        IF (violation == 1) CYCLE
        ...
        DO g = 1, bn
           ...
           ENDIF
        ENDDO
        CALL kkt_check(is_in_E_set, violation, bn, ix, iy, vl, pf, pfl1, bs, lama1,&
        lama2,lama3, ga, nvars,drgix,drgiy,cn,cn_s,cn_e,b) ! Step 4
        IF (violation == 1) CYCLE
        EXIT
...
SUBROUTINE spmat_four (bn,bs,ix,iy,gam,nobs,nvars,x,xidx,xcptr,nnz,y,pf,pfl1,&
     ...
     activeGroup,nbeta,alam,npass,jerr,mse,alsparse1,alsparse2,lb,ub,cn,drgix,drgiy,cn_s,cn_e)
  ! --------------------------------------------------
  USE sgl_subfuns
  USE spmatmul
  IMPLICIT NONE
  ...
  DO WHILE (l < nlam) !! This is the start of the loop over all lambda values...
     ...
     ENDIF
     lama1 = al 
     lama2 = al * alsparse1(1)
     lama3 = al * alsparse2(1)
     ! This is the start of the algorithm, for a given lambda...
     CALL strong_rule (is_in_S_set, ga, pf, tlam) !uses s_set instead of e_set...
     ! --------- outer loop ---------------------------- !
     DO
        ...
        DO
           ...
           DO g = 1, bn
              ...
              CALL sp_update_step(bs(g), startix, endix, b, lama1,lama2,lama3, t_for_s(g),&
                   pf(g), pfl1(startix:endix), x, xidx, xcptr, nnz, isDifZero, nobs,&
                   r, gam(g), maxDif, nvars, lb(g), ub(g),startsubgp, endsubgp, drgix, drgiy, drgdim=cn)
              IF (activeGroupIndex(g) == 0 .AND. isDifZero == 1) THEN
                 ni = ni+1
                 IF (ni > pmax) EXIT
                 activeGroupIndex(g) = ni
        ...
        CALL sp_strong_kkt_check(is_in_E_set, violation, bn, ix, iy, pf, pfl1,&
             bs, lama1,lama2,lama3, ga, is_in_S_set, x, xidx, xcptr, nnz,&
             r,nobs,nvars, vl,drgix,drgiy,cn,cn_s,cn_e,b)
        IF (violation == 1) CYCLE
        ! Need to compute vl/ga for the ones that aren't already updated, before kkt_check
        DO g = 1, bn
           CALL rchkusr()
