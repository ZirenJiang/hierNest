SUBROUTINE wsgl (bn,bs,ix,iy,gam,nobs,nvars,x,r,pf,pfl1,pmax,&
   ...
   npass,jerr,alsparse1,alsparse2,lb,ub,sset,eset,b0old,betaold,al0,findlambda,l,me,cn)
   ...
   DOUBLE PRECISION :: eps, alsparse1,alsparse2, max_gam, d, maxDif, al, snorm
  ...
  INTEGER:: drgiy(cn)
   ...
   DOUBLE PRECISION:: tlam, lama1,lama2,lama3
   INTEGER:: violation
   INTEGER :: sset(bn)
   ...
   CALL strong_rule(sset, ga, pf, tlam) !uses s_set instead of e_set...
   ! --------- outer loop ---------------------------- !
   DO
      ! PRINT *, al
      CALL rchkusr()
      ...
      CALL strong_kkt_check(eset, violation, bn, ix, iy, pf, pfl1,&
            bs, lama1,lama2,lama3, ga, sset, x, r, nobs, nvars, vl,drgix,drgiy,cn,cn_s,cn_e,b) ! Step 3
      IF (violation == 1) CYCLE
      ...
      DO g = 1, bn
         ...
         IF (sset(g) == 0) THEN
            ...
            vl(startix:endix) = s
            CALL softthresh(s, lama1 * pfl1(startix:endix), bs(g))
            snorm = SQRT(DOT_PRODUCT(s,s))
            ga(g) = snorm
            DEALLOCATE(s)
         ENDIF
      ENDDO
      CALL kkt_check(eset, violation, bn, ix, iy, vl, pf, pfl1, bs, lama1,&
...
SUBROUTINE spmat_wsgl (bn,bs,ix,iy,gam,nobs,nvars,x,xidx,xcptr,nnz,r,pf,pfl1,pmax,&
   ...
   INTEGER:: activeGroup(pmax)
   DOUBLE PRECISION :: eps, alsparse1,alsparse2, max_gam, d, maxDif, al, alf, snorm
   ...
   DO
      ...
      DO
         ...
         DO g = 1, bn
            ...
            CALL sp_update_step(bs(g), startix, endix, b, lama1,lama2,lama3, t_for_s(g),&
                   ...
                   r, gam(g), maxDif, nvars, lb(g), ub(g),startsubgp=startsubgp, endsubgp=endsubgp, drgix=drgix, &
                  drgiy=drgiy, drgdim=drgdim)
            IF (activeGroupIndex(g) == 0 .AND. isDifZero == 1) THEN
               ni = ni + 1
               IF (ni > pmax) EXIT
      ...
      CALL sp_strong_kkt_check(eset, violation, bn, ix, iy, pf, pfl1,&
            bs, lama1,lama2,lama3, ga, sset, x, xidx, xcptr, nnz,&
